...
test("workspace launch config appends safe OMP arguments", async () => {
  ...
  assert.match(source, /OMP_PROFILE: profile/);
  assert.match(source, /PI_PROFILE: profile/);
});
...
test("buildSessionSpawnArgs applies workspace launch config safely", async () => {
  ...
  assert.deepEqual(
    buildSessionSpawnArgs("", undefined, false, { profile: "work", extraArgs: ["--verbose"] }),
    ["--profile", "work", "--verbose"],
  );
  // 恢复会话时 profile 必须位于 --resume 前，OMP 才会从正确 profile 读取记录。
  assert.deepEqual(
    buildSessionSpawnArgs("/tmp/work-session.jsonl", undefined, false, { profile: "work" }),
    ["--profile", "work", "--resume", "/tmp/work-session.jsonl"],
  );
  // 仅当前对话的 advisor 选择可加 --advisor；保留参数永远不会传给子进程。
  assert.deepEqual(
    buildSessionSpawnArgs("", undefined, true, { extraArgs: ["--advisor", "--cwd=/evil", "--resume", "--verbose"] }),
    ["--advisor", "--verbose"],
  );
...
test("new sessions switch away from OMP's implicit recent session after ready", async () => {
  const source = await readFile(new URL("./rpc-manager.ts", import.meta.url), "utf8");
  const startup = source.slice(
    source.indexOf("const created = new AgentSessionWrapper"),
    source.indexOf("const realSessionId =", source.indexOf("const created = new AgentSessionWrapper")),
  );
  assert.match(startup, /await created\.waitUntilReady\(\)/);
  assert.match(startup, /if \(!sessionFile\)/);
  assert.match(startup, /await created\.send\(\{ type: "new_session" \}\)/);
  assert.ok(startup.indexOf("await created.waitUntilReady()") < startup.indexOf('await created.send({ type: "new_session" })'));
});
