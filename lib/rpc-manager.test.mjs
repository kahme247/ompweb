...
test("rpc-manager defines DISCONNECT_DESTROY_MS and MAX_ACTIVE_SESSIONS contracts", async () => {
  const source = await readFile(new URL("./rpc-manager.ts", import.meta.url), "utf8");
  assert.match(source, /const DISCONNECT_DESTROY_MS =/);
  assert.match(source, /const MAX_ACTIVE_SESSIONS =/);
  assert.match(source, /export async function evictStaleRpcSessions/);
  assert.match(source, /await evictStaleRpcSessions\(\)/);
});

test("evictStaleRpcSessions evicts oldest idle session and preserves running sessions", async (t) => {
  const { evictStaleRpcSessions: evictFn, AgentSessionWrapper: TestWrapper } = await runtimeJiti.import("./rpc-manager.ts");
  assert.equal(typeof evictFn, "function");

  const s1 = snapshotSession(t, async () => ({}), TestWrapper).wrapper;
  const s2 = snapshotSession(t, async () => ({}), TestWrapper).wrapper;
  const s3 = snapshotSession(t, async () => ({}), TestWrapper).wrapper;

  s1.lastActiveTime = 1000;
  s2.lastActiveTime = 2000;
  s3.lastActiveTime = 3000;

  // With limit = 10, nothing is evicted
  const evicted0 = await evictFn(10);
  assert.equal(evicted0, 0);
  assert.equal(s1.isAlive(), true);
  assert.equal(s2.isAlive(), true);
  assert.equal(s3.isAlive(), true);
});

test("disconnecting all listeners schedules quick cleanup timer", async (t) => {
  const { wrapper } = snapshotSession(t);
