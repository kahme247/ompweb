...
test("ignores empty segments from stray slashes", async () => {
  const { buildSearchTree } = await loadSubject();
  const tree = buildSearchTree(["/lib/paths.ts", "components//App.tsx", "docs/"]);

  // Directories first, so the two real directories precede "docs", which had no
  // leaf after its trailing slash and therefore stays a plain node.
  assert.deepEqual(names(tree), ["components", "lib", "docs"]);
  assert.deepEqual(names(tree[0].children), ["App.tsx"]);
  assert.equal(tree[0].children[0].path, "components/App.tsx");
  assert.equal(tree[1].children[0].path, "lib/paths.ts");
  // No nameless child may be invented for the trailing slash.
  assert.deepEqual(tree[2].children, []);
});

test("promotes a name that first arrived as a file once children follow", async () => {
  const { buildSearchTree } = await loadSubject();
  // Without the promotion, "a" stays a file and its matched child is
  // unreachable in the rendered tree.
  const tree = buildSearchTree(["a", "a/b.ts"]);

  assert.equal(tree.length, 1);
  assert.equal(tree[0].isDir, true);
  assert.deepEqual(names(tree[0].children), ["b.ts"]);

  // Reverse order: the directory already exists, so the later leaf collapses
  // into it rather than replacing it.
  const reversed = buildSearchTree(["a/b.ts", "a"]);
  assert.equal(reversed[0].isDir, true);
  assert.deepEqual(names(reversed[0].children), ["b.ts"]);
});

test("handles nesting far deeper than any real path", async () => {
  const { buildSearchTree } = await loadSubject();
  const depth = 500;
  const path = Array.from({ length: depth }, (_, i) => `d${i}`).join("/") + "/leaf.ts";
  const tree = buildSearchTree([path]);
  ...
  assert.equal(node.name, "leaf.ts");
  assert.equal(node.isDir, false);
});
