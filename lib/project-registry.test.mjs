...
test("preserves display aliases, sort order, and launch config", () => {
  ...
  const withLaunchConfig = updateProjectsPresentation(updated, [{ path: P("/one"), launchConfig: { profile: "work", extraArgs: ["--verbose"] } }]);
  assert.deepEqual(withLaunchConfig.projects[0].launchConfig, { profile: "work", extraArgs: ["--verbose"] });
});
test("strips reserved and flag-smuggled launch args from disk configs", () => {
  ...
  assert.equal(isReservedLaunchArg("--advisor"), true);
  ...
  const registry = parseProjectRegistry(JSON.stringify({ version: 1, projects: [{
    ...
    launchConfig: { profile: "--evil", advisor: true, extraArgs: ["--advisor", "--verbose", "--cwd=/evil", "--resume"] },
  }] }));
  // 无效 profile、保留参数和旧版 advisor 设置都会被忽略。
  assert.deepEqual(registry.projects[0].launchConfig, { profile: undefined, extraArgs: ["--verbose"] });
});
test("re-adding a workspace without config preserves its launch config", () => {
  ...
  assert.deepEqual(revived.projects[0].launchConfig, { profile: "work", extraArgs: undefined });
  const replaced = upsertProject(seeded, P("/one"), "2026-02-01T00:00:00.000Z", { extraArgs: ["--verbose"] });
  assert.deepEqual(replaced.projects[0].launchConfig, { extraArgs: ["--verbose"] });
});
