...
test("loads a workspace profile from its profile-specific session directory", async (t) => {
  ...
  try {
    ...
    const defaultSessions = join(defaultAgentDir, "sessions", "-workspace");
    const profileSessions = join(getSessionsDir("work"), "-workspace");
    mkdirSync(defaultSessions, { recursive: true });
    mkdirSync(profileSessions, { recursive: true });
    writeSessionFile(defaultSessions, "default.jsonl", { id: "same-id", cwd: workspace, timestamp: "2026-01-01T00:00:00.000Z" });
    writeSessionFile(profileSessions, "profile.jsonl", { id: "same-id", cwd: workspace, timestamp: "2026-01-02T00:00:00.000Z" });
