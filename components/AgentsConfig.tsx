...
import { AlertCircle, Bot, Check, Copy, Plus, RefreshCw, Search, Trash2, ChevronLeft, ChevronRight } from "lucide-react";
...
export function AgentsConfig({ cwd }: { cwd: string | null }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
      ...
      {isMobile ? (
        creating || active ? (
          <div style={{ border: "1px solid var(--border)", borderRadius: "var(--radius-card)", background: "var(--bg-panel)", overflow: "hidden", display: "flex", flexDirection: "column" }}>
            ...
            {isBundledActive ? (
              <div style={{ padding: "8px 12px", background: "color-mix(in srgb, var(--accent) 10%, var(--bg-panel))", borderBottom: "1px solid var(--border)", fontSize: 11, color: "var(--text-muted)", display: "flex", alignItems: "center", gap: 6 }}>
            ...
            <div style={{ padding: 14, display: "flex", flexDirection: "column", gap: 12 }}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap", marginTop: 4 }}>
                <button type="button" onClick={() => void save()} disabled={saving || workspaceCheckPending || activeProjectUnavailable} style={{ flex: 1, padding: "8px 14px", border: "1px solid var(--accent)", borderRadius: "var(--radius-control)", background: "var(--accent)", color: "white", cursor: saving || workspaceCheckPending || activeProjectUnavailable ? "not-allowed" : "pointer", display: "inline-flex", alignItems: "center", justifyContent: "center", gap: 6, fontSize: 13, fontWeight: 600 }}>
                  <Check size={14} aria-hidden="true" /> {creating ? t("agentsConfig.create") : t("agentsConfig.save")}
                </button>
                {!creating && (
                  <button type="button" onClick={() => void remove()} disabled={saving || isBundledActive || activeProjectUnavailable} style={{ padding: "8px 14px", border: "1px solid var(--border)", borderRadius: "var(--radius-control)", background: "var(--bg)", color: isBundledActive || activeProjectUnavailable ? "var(--text-dim)" : "var(--status-error, #e5484d)", cursor: isBundledActive || saving || activeProjectUnavailable ? "not-allowed" : "pointer", display: "inline-flex", alignItems: "center", gap: 6, fontSize: 13 }}>
                    <Trash2 size={14} aria-hidden="true" /> {t("agentsConfig.remove")}
                  </button>
                )}
              </div>
          ...
          <div style={{ border: "1px solid var(--border)", borderRadius: "var(--radius-card)", background: "var(--bg-panel)", overflow: "hidden", display: "flex", flexDirection: "column" }}>
            <div style={{ padding: "10px 12px", borderBottom: "1px solid var(--border)", fontSize: 12, color: "var(--text-muted)", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
              <span>{tn("agentsConfig.agentsCount", filtered.length)}</span>
              <button
                type="button"
                onClick={startCreate}
                disabled={workspaceCheckPending}
                style={{
                  display: "inline-flex",
                  alignItems: "center",
                  gap: 4,
              ...
              {filtered.length === 0 ? (
                ...
                filtered.map((a) => {
                  ...
                  return (
                    ...
                    >
                      <div style={{ display: "flex", gap: 10, alignItems: "flex-start", flex: 1, minWidth: 0, paddingRight: 8 }}>
                        ...
                        <div style={{ minWidth: 0, flex: 1 }}>
                          <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                            ...
                            <span style={{ fontSize: 10, padding: "1px 6px", borderRadius: 999, background: badgeBg, color: "var(--text-muted)", border: "1px solid var(--border)", flexShrink: 0 }}>{a.scope}</span>
                            {!a.valid ? <AlertCircle size={11} aria-hidden="true" style={{ color: "var(--status-error, #e5484d)" }} /> : null}
                          </div>
                          <div style={{ marginTop: 2, fontSize: 11.5, color: "var(--text-muted)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{a.description || "—"}</div>
                        </div>
                      </div>
