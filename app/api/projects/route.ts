...
const RESERVED_LAUNCH_ARGS = new Set(["--mode", "rpc-ui", "--cwd", "--resume"]);
...
function parseLaunchConfig(value: unknown): ProjectLaunchConfig | undefined {
  ...
  const extraArgs = raw.extraArgs === undefined ? undefined : (raw.extraArgs as unknown[]).map((arg) => {
    if (typeof arg !== "string" || !arg || arg.length > MAX_EXTRA_ARG_LENGTH || RESERVED_LAUNCH_ARGS.has(arg)) throw new ProjectPathError("invalid_extra_args", "Extra args contain an invalid or reserved argument");
    return arg;
  });
